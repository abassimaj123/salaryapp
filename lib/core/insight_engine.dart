...
class InsightEngine {
  ...
  static List<Insight> generate({
    ...
    required double federalBracketPct, // top bracket rate as decimal (e.g. 0.22)
    bool isEs = false,
    bool isFr = false,
    int maxCount = 3,
  }) {
    ...
    if (effectivePct > 35) {
      insights.add(Insight(
        ...
        title: isFr ? 'Fardeau fiscal élevé'
               : isEs ? 'Alta carga fiscal'
               : 'High Tax Burden',
        body: isFr
            ? 'Taux effectif élevé : ${effectivePct.toStringAsFixed(1)}% — envisagez des déductions supplémentaires.'
            : isEs
            ? 'Tasa efectiva alta: ${effectivePct.toStringAsFixed(1)}% — considera maximizar deducciones.'
            : 'High tax burden: effective rate ${effectivePct.toStringAsFixed(1)}% — consider maximizing deductions.',
      ));
    } else if (effectivePct >= 25) {
      insights.add(Insight(
        severity: InsightSeverity.warning,
        title: isFr ? 'Charge fiscale modérée'
               : isEs ? 'Carga fiscal moderada'
               : 'Moderate Tax Burden',
        body: isFr
            ? 'Taux effectif modéré : ${effectivePct.toStringAsFixed(1)}%.'
            : isEs
            ? 'Carga fiscal moderada: tasa efectiva ${effectivePct.toStringAsFixed(1)}%.'
            : 'Moderate tax burden: effective rate ${effectivePct.toStringAsFixed(1)}%.',
      ));
    } else {
      insights.add(Insight(
        severity: InsightSeverity.good,
        title: isFr ? 'Taux d\'imposition sain'
               : isEs ? 'Tasa fiscal saludable'
               : 'Healthy Tax Rate',
        body: isFr
            ? 'Taux effectif de ${effectivePct.toStringAsFixed(1)}% — bonne position fiscale.'
            : isEs
            ? 'Tasa efectiva de ${effectivePct.toStringAsFixed(1)}% — buena posición fiscal.'
            : 'Healthy effective rate: ${effectivePct.toStringAsFixed(1)}%.',
      ));
    }
    ...
    insights.add(Insight(
      severity: InsightSeverity.good,
      title: isFr ? 'Salaire net conservé'
             : isEs ? 'Salario neto retenido'
             : 'Take-Home Share',
      body: isFr
    ...
    if (federalBracketPct >= 0.24) {
      ...
      insights.add(Insight(
        severity: InsightSeverity.warning,
        title: isFr ? 'Tranche marginale élevée'
               : isEs ? 'Tramo federal alto'
    ...
    if (grossAnnual >= 20000) {
      ...
      insights.add(Insight(
        severity: InsightSeverity.good,
        title: isFr ? 'Décomposition du revenu net'
               : isEs ? 'Desglose de ingreso neto'
               : 'Net Pay Breakdown',
        body: isFr
    ...
    if (ficaTax > 0 && grossAnnual > 0) {
      ...
      insights.add(Insight(
        severity: InsightSeverity.good,
        title: isFr ? 'FICA (RAS + Assurance maladie)'
               : isEs ? 'Contribuciones FICA'
               : 'FICA Contributions',
        body: isFr
    ...
    if (ordered.isEmpty) {
      ordered.add(Insight(
        severity: InsightSeverity.good,
        title: isFr ? 'Calcul Terminé'
               : isEs ? 'Cálculo Completado'
