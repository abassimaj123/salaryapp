...
class InsightEngine {
  ...
  static List<Insight> generate({
    ...
    bool isEs = false,
    // ignore isFr — this engine is US-only; kept for API compat but unused.
    bool isFr = false,
    int maxCount = 3,
  }) {
    ...
    if (effectivePct > 35) {
      insights.add(Insight(
        ...
        title: isEs ? 'Alta carga fiscal' : 'High Tax Burden',
        body: isEs
      ...
      insights.add(Insight(
        ...
        title: isEs ? 'Carga fiscal moderada' : 'Moderate Tax Burden',
        body: isEs
      ...
      insights.add(Insight(
        ...
        title: isEs ? 'Tasa fiscal saludable' : 'Healthy Tax Rate',
        body: isEs
    ...
    insights.add(Insight(
      severity: InsightSeverity.good,
      title: isEs ? 'Salario neto retenido' : 'Take-Home Share',
      body: isEs
    ...
    if (federalBracketPct >= 0.24) {
      ...
      insights.add(Insight(
        severity: InsightSeverity.warning,
        title: isEs ? 'Tramo federal alto' : 'High Federal Bracket',
        body: isEs
    ...
    if (grossAnnual >= 20000) {
      ...
      insights.add(Insight(
        severity: InsightSeverity.good,
        title: isEs ? 'Desglose de ingreso neto' : 'Net Pay Breakdown',
        body: isEs
    ...
    if (ficaTax > 0 && grossAnnual > 0) {
      ...
      insights.add(Insight(
        severity: InsightSeverity.good,
        title: isEs ? 'Contribuciones FICA' : 'FICA Contributions',
        body: isEs
